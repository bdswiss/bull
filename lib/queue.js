...
function redisOptsFromUrl(urlString) {
  ...
  try {
    ...
    if (redisUrl.protocol === 'rediss:') {
      redisOpts.tls = {};
      if (redisUrl.query) {
        if (redisUrl.query.ca) {
          redisOpts.tls.ca = fs.readFileSync(redisUrl.query.ca);
        }
        if (redisUrl.query.cert) {
          redisOpts.tls.cert = fs.readFileSync(redisUrl.query.cert);
        }
        if (redisUrl.query.key) {
          redisOpts.tls.key = fs.readFileSync(redisUrl.query.key);
        }
        if (redisUrl.query.rejectUnauthorized) {
          redisOpts.tls.rejectUnauthorized =
            redisUrl.query.rejectUnauthorized === 'true';
        }
      }
    }

    if (redisUrl.query) {
      if (redisUrl.query.maxRetriesPerRequest) {
        redisOpts.maxRetriesPerRequest = parseInt(
          redisUrl.query.maxRetriesPerRequest,
          10
        );
      }
      if (redisUrl.query.connectTimeout) {
        redisOpts.connectTimeout = parseInt(redisUrl.query.connectTimeout, 10);
      }
      if (redisUrl.query.keepAlive) {
        redisOpts.keepAlive = parseInt(redisUrl.query.keepAlive, 10);
      }
      if (redisUrl.query.autoResubscribe) {
        redisOpts.autoResubscribe = redisUrl.query.autoResubscribe === 'true';
      }
    }
